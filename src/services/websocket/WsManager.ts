//WsManager is the core engine. It owns the WebSocket connection, handles everything that can go wrong with it, and notifies listeners when data arrives. It is the only place in the codebase that touches the native WebSocket API.

// ─────────────────────────────────────────────────────────────────────────────
// services/websocket/WsManager.ts
// Production-grade singleton that owns the live-market WebSocket connection.
//
// Responsibilities
//   • Build authenticated WSS URL (clientCode from bearer token)
//   • Connect / disconnect lifecycle
//   • Exponential back-off reconnection with jitter + max-attempt guard
//   • Heartbeat (PING every 25 s, PONG timeout detection → force-reconnect)
//   • Subscription management (re-subscribe after reconnect)
//   • Multi-listener tick & lifecycle event fan-out
// ─────────────────────────────────────────────────────────────────────────────
